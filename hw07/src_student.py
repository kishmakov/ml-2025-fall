...
class ExperimentHandler:
    ...
    def generate_stacking_predictions(self, estimator, run):
        ...
        returns fitted estimator for convenience and less train overhead
...
class MyRandomForestClassifier:
    ...
    def predict_proba(self, X: np.ndarray):
        ...
        """
        X2 = np.atleast_2d(X)
        sum_probas = np.zeros((X2.shape[0], self._n_classes))
        for estimator in self.estimators:
            probas = estimator.predict_proba(X2)
            # pad in case a bootstrap missed some classes
            if probas.shape[1] < self._n_classes:
                pad = self._n_classes - probas.shape[1]
                probas = np.hstack([probas, np.zeros((X2.shape[0], pad))])
            sum_probas += probas
        sum_probas /= len(self.estimators)
        return sum_probas
